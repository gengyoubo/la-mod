...
public abstract class BIAbstractFurnaceMenu extends RecipeBookMenu<Container> {
    ...
    protected BIAbstractFurnaceMenu(MenuType<?> p_38960_, RecipeType<? extends AbstractCookingRecipe> p_38961_, RecipeBookType p_38962_, int p_38963_, Inventory p_38964_) {
        this(p_38960_, p_38961_, p_38962_, p_38963_, p_38964_, new SimpleContainer(3), new SimpleContainerData(4));
    }

    protected BIAbstractFurnaceMenu(MenuType<?> p_38966_, RecipeType<? extends AbstractCookingRecipe> p_38967_, RecipeBookType p_38968_, int p_38969_, Inventory p_38970_, Container p_38971_, ContainerData p_38972_) {
        ...
        this.addSlot(new BISlot(p_38971_, 0, 56, 17));
        this.addSlot(new BIFurnaceFuelSlot(this, p_38971_, 1, 56, 53));
        this.addSlot(new FurnaceResultSlot(p_38970_.player, p_38971_, 2, 116, 35));

        int k;
        for(k = 0; k < 3; ++k) {
            for(int j = 0; j < 9; ++j) {
                this.addSlot(new Slot(p_38970_, j + k * 9 + 9, 8 + j * 18, 84 + k * 18));
            }
        }
    ...
    protected boolean canSmelt(ItemStack p_38978_) {
        return this.level.getRecipeManager().getRecipeFor(this.recipeType, new SimpleContainer(new ItemStack[]{p_38978_}), this.level).isPresent();
    }

    protected boolean isFuel(ItemStack p_38989_) {
        return ForgeHooks.getBurnTime(p_38989_, this.recipeType) > 0;
    }
